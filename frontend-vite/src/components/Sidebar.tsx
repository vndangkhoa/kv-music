          ...
          {[
            ...
            <Link key={to} to={to}
              className={`flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] font-medium transition ${
                isActive(to) ? 'bg-white/10 text-white' : 'text-neutral-400 hover:text-white hover:bg-white/5'
              }`}>
      ...
      <div className="px-3 py-2">
        <Link to="/collection/tracks" className="block group">
          <div className={`flex items-center gap-3 px-3 py-2 rounded-lg transition ${
            isActive('/collection/tracks') ? 'bg-white/10' : 'hover:bg-white/5'
          }`}>
        ...
        <div className="px-3 mb-1">
          <div className="flex items-center justify-between mb-2 px-1">
            ...
            <button onClick={() => setIsCreateModalOpen(true)}
              className="p-1 -mr-1 rounded-md text-neutral-500 hover:text-white hover:bg-white/10 transition" title="Create Playlist">
              ...
              return (
                <button key={filter} onClick={() => setActiveFilter(active ? 'all' : key)}
                  className={`px-3 py-1 rounded-md text-xs font-medium transition ${
                    active ? 'bg-white text-black' : 'bg-white/[0.06] text-neutral-400 hover:bg-white/10 hover:text-white'
                  }`}>
                  {filter}
                </button>
              );
          ...
          <div className="space-y-0.5">
              ...
              return (
                <div key={playlist.id} className={`group relative rounded-lg transition ${active ? 'bg-white/10' : ''}`}>
                  <Link to={`/playlist/${playlist.id}`}
                    ...
                    <CoverImage src={playlist.cover_url} alt={playlist.title} className="w-9 h-9 rounded-md flex-shrink-0 object-cover" />
            ...
            {showArtists && followedArtists.map((artistName) => (
              <Link key={artistName} to={`/artist/${encodeURIComponent(artistName)}`}
                ...
                <CoverImage
                  ...
                  alt={artistName} className="w-9 h-9 rounded-full flex-shrink-0 object-cover"
            ...
            {showAlbums && savedAlbums.map((album) => (
              <Link key={album.id} to={`/album/${album.id}`}
                className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-white/5 transition">
                <CoverImage src={album.cover_url} alt={album.title} className="w-9 h-9 rounded-md flex-shrink-0 object-cover" fallbackText="&#128191;" />
                <div className="flex-1 min-w-0">
